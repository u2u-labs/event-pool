...
const (
	...
	methodBan                  = "ban"
	methodCheckStateValidators = "checkStateValidators"
)
...
func (p *TxPool) processEvent(event *blockchain.Event) {
	...
	updateStateNonces := func(address types.Address) bool {
		// skip already processed accounts
		if _, processed := stateNonces[address]; processed {
			return processed
		}

		// fetch latest nonce from the state
		latestNonce := p.store.GetNonce(stateRoot, address)

		// update the result map
		stateNonces[address] = latestNonce
		return false
	}
	...
	for _, header := range event.NewChain {
		...
		for _, tx := range block.Transactions {
			...
			if processed := updateStateNonces(addr); processed {
				continue
			}
		...
		if processed := updateStateNonces(block.Header.Creator); processed {
			continue
		}
	}
...
func (p *TxPool) handleEnqueueRequest(req enqueueRequest) {
	...
	if tx.Nonce > account.getNonce() {
		...
		// higher nonce txs
		p.logger.Debug("enqueue failed", "err", "incorrect nonce", "hash", tx.Hash.String(), "tx", tx.Nonce, "account", account.getNonce())
		return
	}
