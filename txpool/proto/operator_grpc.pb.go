...
func (c *txnPoolOperatorClient) Status(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*TxnPoolStatusResp, error) {
	...
	err := c.cc.Invoke(ctx, TxnPoolOperator_Status_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *txnPoolOperatorClient) Subscribe(ctx context.Context, in *SubscribeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[TxPoolEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &TxnPoolOperator_ServiceDesc.Streams[0], TxnPoolOperator_Subscribe_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeRequest, TxPoolEvent]{ClientStream: stream}
...
type TxnPoolOperator_SubscribeClient = grpc.ServerStreamingClient[TxPoolEvent]
...
// UnimplementedTxnPoolOperatorServer must be embedded to have
...
func (UnimplementedTxnPoolOperatorServer) testEmbeddedByValue()                         {}
...
func RegisterTxnPoolOperatorServer(s grpc.ServiceRegistrar, srv TxnPoolOperatorServer) {
	// If the following call pancis, it indicates UnimplementedTxnPoolOperatorServer was
...
func _TxnPoolOperator_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(TxnPoolOperatorServer).Subscribe(m, &grpc.GenericServerStream[SubscribeRequest, TxPoolEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TxnPoolOperator_SubscribeServer = grpc.ServerStreamingServer[TxPoolEvent]
