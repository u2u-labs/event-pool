...
func (i *backendIBFT) IsValidBlock(proposal []byte) bool {
	...
	if err := i.blockchain.VerifyPotentialBlock(newBlock, i.currentValidators); err != nil {
...
func (i *backendIBFT) IsValidSender(msg *protoIBFT.IBFTMessage) bool {
	msgNoSig, err := msg.PayloadNoSig()
	if err != nil {
		if view := msg.View; i.IsEpochHeight(view.Height) && msg.Type == protoIBFT.MessageType_PREPREPARE {
			i.logger.Debug("failed to recreate message", "err", err, "h", view.Height, "round", view.Round,
...
func (i *backendIBFT) IsProposer(id []byte, height, round uint64) bool {
	...
	// Calculate the next proposer based on the validators staking information, round, and the previous proposer.
	nextProposer := i.CalcNextProposer(height, round)
	if nextProposer == nil {
		return false
	}

	return types.BytesToAddress(id) == types.BytesToAddress(nextProposer)
}

// CalcNextProposer function to find next proposer and check state of next proposer
func (i *backendIBFT) CalcNextProposer(height uint64, round uint64) []byte {
	var nextProposer validators.Validator
	...
	//create a loop to find next proposer
	...
	return nextProposer.Addr().Bytes()
}

//func (i *backendIBFT) getTransitionForQuery(height uint64) (*state.Transition, error) {
//	header, ok := i.blockchain.GetHeaderByNumber(height)
//	if !ok {
//		return nil, fmt.Errorf("header not found at %d", height)
//	}
//
//	return i.executor.BeginTxn(header.StateRoot, header, types.ZeroAddress)
//}
