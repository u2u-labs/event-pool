...
type syncPeerClient struct {
	logger     *zap.SugaredLogger // logger used for console logging
...
func (m *syncPeerClient) startNewBlockProcess() {
	...
	for e := range m.subscription.GetEventCh() {
		if !m.shouldEmitBlocks {
			continue
		}

		if l := len(e.NewChain); l > 0 {
			latest := e.NewChain[l-1]
			// Publish status
	...
	go func() {
		...
		for {
			...
			case err = <-streamErrorCh:
				m.logger.Errorw("failed to get block from gRPC stream", "peer", peerID, "err", err)

				return
			case <-time.After(timeoutPerBlock):
				m.logger.Warnw("block doesn't reach within timeout", "timeout", timeoutPerBlock)
