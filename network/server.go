...
import (
	...
	"event-pool/network/dial"
	"event-pool/network/discovery"
...
type Server struct {
	logger *zap.SugaredLogger // the logger
...
func (s *Server) Start() error {
	s.logger.Infow("LibP2P server running", "addr", common.AddrInfoToString(s.AddrInfo()))
...
func (s *Server) GetProtocols(peerID peer.ID) ([]string, error) {
	_protocolIDs, err := s.host.Peerstore().GetProtocols(peerID)
	if err != nil {
		return nil, err
	}
	protocolIDs := make([]string, 0)
	for _, id := range _protocolIDs {
		protocolIDs = append(protocolIDs, string(id))
	}
